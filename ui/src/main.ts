...
import { updateFileCount, updatePathsList, updateFileList, updateProgressBar, showProgressBar, showLogSection, clearLogSection, addSpacerToLog, updateLog, prepareTabs } from './dom';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    listen<[string, number, string[]]>('job-finished', event => {
        ...
        updateLog(logLine, totalFiles !== successfulFiles ? LogSeverity.ERROR : LogSeverity.INFO);

        const failedPaths: string[] = event.payload[2];
        if (failedPaths.length > 0) {
            addSpacerToLog();

            const staticLine: string = 'The following files were not processed due to the errors above:';
